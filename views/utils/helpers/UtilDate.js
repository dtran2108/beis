...
export default class UtilDate {
  ...
  static formatDateTimeLocal1 = 'DD/MM/YYYY HH:mm ';
  static formatDayLocal = 'DD/MM';
    ...
    try {
      ...
      if (isUTC) {
        /*
                ...
                */
        return moment.utc(dateTime).local().lang('vi').format(UtilDate.formatDateTimeLocal1);
      } else {
        return moment(dateTime).format(UtilDate.formatDateTimeLocal1);
      }
    } catch (e) {
      return undefinedString;
    }
  ...
  static toDayLocal(dateTime, isUTC = UtilDate.DEFAULT_UTC, stringFormatDate = UtilDate.formatDayLocal) {
    try {
      if (!UtilDate.isValid(dateTime) || _.isNil(dateTime)) {
        return '';
      }
  ...
  static toDateTimeUtc(dateTimeLocal, isUTC = UtilDate.DEFAULT_UTC) {
    try {
      if (!UtilDate.isValid(dateTimeLocal)) {
        return '';
      }
      if (isUTC) {
        ...
        return moment(dateTimeLocal).format(UtilDate.formatDateTimeServer);
      }
    } catch (e) {
      return '';
    }
  ...
  static getRangeDate(dateRange) {
    ...
    switch (dateRange) {
      ...
      case 'week':
        current = new Date();
        startOfDay = new Date(current.getFullYear(), current.getMonth(), current.getDate());
        end = new Date(current.getFullYear(), current.getMonth(), current.getDate(), 23, 59, 59, 999);
        startOfWeek = moment(startOfDay).add(-1 * startOfDay.getDay() + 1, 'days');
      ...
      case 'year':
        current = new Date();
        startOfYear = new Date(current.getFullYear(), 0, 1);
        end = new Date(current.getFullYear(), current.getMonth(), current.getDate(), 23, 59, 59, 999);
